...
func TestDaemonSetNoChange(t *testing.T) {
	...
	flp := getFLPConfig()
	loki := getLokiConfig()
	b := newBuilder(ns, corev1.ProtocolUDP, &flp, &loki, nil, ConfSingle)
...
func TestDeploymentChanged(t *testing.T) {
	...
	flp2 := flp
	flp2.Replicas = 5
	b = newBuilder(ns, corev1.ProtocolUDP, &flp2, &loki, nil, ConfSingle)
	_, digest = b.configMap()
...
func TestLabels(t *testing.T) {
	...
	gfk := getFLPConfig()
	builder := newBuilder("ns", corev1.ProtocolUDP, &gfk, nil, nil, ConfSingle)
